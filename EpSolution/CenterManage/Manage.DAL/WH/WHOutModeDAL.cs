...
namespace Manage.DAL.WH
{
    ...
    ///　编写日期：2017年07月02日
    /// </summary>
    public class WHOutModeDAL : BaseDAL
    {
        #region 获取信息
        /// <summary>
        /// 获取信息
        ...
        public List<WHOutMode> GetList(WHOutMode condition)
        {
            List<WHOutMode> list = null;
            string sql = null;

            using (IDataSession session = AppDataFactory.CreateMainSession())
            {
                sql = "SELECT ID,Description FROM T_WH_OutMode";
                list = session.GetList<WHOutMode>(sql, new List<DataParameter>().ToArray()).ToList<WHOutMode>();
            }
        ...
        public DataPage GetList(WHOutMode condition, DataPage page)
        {
            ...
            try
            {
                ...
                using (IDataSession session = AppDataFactory.CreateMainSession())
                {
                    page = session.GetDataPage<WHOutMode>(sql, parameters.ToArray(), page);
                }
                return page;
            }
            catch (Exception ex)
        ...
        private string GetQuerySql(WHOutMode condition, ref List<DataParameter> parameters)
        {
            ...
            try
            {
                ...
                sqlBuilder.Append("FROM T_WH_OutMode ");
        ...
        public int Insert(WHOutMode model)
        {
            int count = 0;
            try
            {
                ...
                {
                    //插入基本信息
                    count = session.Insert<WHOutMode>(model);
                }
                return count;
            }
            catch (Exception ex)
        ...
        public int Update(WHOutMode model)
        {
            int count = 0;
            try
            {
                ...
                {
                    //更新基本信息
                    count = session.Update<WHOutMode>(model);
                }
                return count;
            }
            catch (Exception ex)
