...
namespace Manage.DAL.WH
{
    ...
    public class InStockQueryDAL
    {
        ...
        {
            List<InStockQueryResult> list = null;
            List<DataParameter> parameters=new List<DataParameter>();
            string sql = null;

            sql = @"select t1.Warehouse as WarehouseID,t2.MatID,sum(t2.MainUnitAmount) as Amount
                    from T_WH_InStockBill t1
                    inner join T_WH_InStockDetail t2 on t1.ID = t2.BillID
            ...
            using (IDataSession session = AppDataFactory.CreateMainSession())
            {
                list = session.GetList<InStockQueryResult>(sql, parameters.ToArray()).ToList();
            }
